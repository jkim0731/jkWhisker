...
cd(['Z:\Data\Video\AHHD_006\AH0669\170317'])
...
parfor n=1:length(measures)
    ...
    system(['measure ' '--face ' 'botton ' measures(n).name ' ' outputFileName '.measurements']);
...
parfor n=1:length(classes)
    ...
    system(['classify ' classes(n).name ' ' outputFileName '.measurements ' 'bottom ' '--px2mm ' '0.035 ' '-n ' '-2' '--limit3.0:50.0' ]);
...
parfor n=1:length(classes)
    [~, outputFileName] = fileparts(classes(n).name);
    system(['reclassify ' classes(n).name ' ' outputFileName '.measurements' ' ' '-n ' '2']);
