...
for wl_array_ind = 1 : length(trial_types)
    figure, hold all
    sp1 = []; sp2 = []; sp3 = []; sp4 = []; sp5 = []; sp6 = []; %subplot 1~5
    for wl_ind = 1 : length(wl_array{wl_array_ind}.trials)
        ...
        sp2 = [sp2; wt.deltaKappa{1}(wt.th_touch_frames)', wt.thetaAtBase{2}(wt.th_touch_frames)'];
    ...
    plot(sp1(:,1),sp1(:,2),'k.'), title('top-view theta vs front-view theta');
    subplot(3,2,2), % top-view kappa vs front-view kappa
    plot(sp2(:,1),sp2(:,2),'k.'), title('top-view kappa vs front-view kappa');
    subplot(3,2,3) % top-view theta vs top-view kappa
    plot(sp3(:,1),sp3(:,2),'k.'), title('top-view theta vs kappa');
    subplot(3,2,4) % front-view theta vs front-view kappa
    plot(sp4(:,1),sp4(:,2),'k.'), title('front-view theta vs kappa');
    subplot(3,2,5) % top-view theta vs front-view kappa
    plot(sp5(:,1),sp5(:,2),'k.'), title('top-view theta vs front-view kappa');
    subplot(3,2,6) % top-view theta vs top-view intersection coordinate
    plot(sp6(:,1),sp6(:,2),'k.'), title('top-view theta vs top-view ISC');    
end
