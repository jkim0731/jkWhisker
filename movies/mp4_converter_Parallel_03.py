...
def process_mp4_file(myfile):
    print "Starting process"
    global globcount
    ncpus = multiprocessing.cpu_count()

    while (globcount == ncpus):
        print "Waiting for the slow cpu to finish"
...
def parse_directory(arg, dirname, names):
    ...
    try:
        ...
        if seq_count > 0 & new_count == 0:
            ...
            # relArgs = numpy.array([pathname, seqnames, dirname])
            pool = Pool()
            print "In the pool"
            pool.map(process_mp4_file, seqnames)
        # pool.close()
