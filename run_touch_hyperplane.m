...
for iservo = 1 : length(servo_values)
    for idist = 1 : length(distance_values) 
        ...
        psi2Flip = 0;
        if length(unique(apPosition)) == 1 % try in 2D plot
            ind_opt = 1;
            intersect_3d_total = [];
            for tnum = 1 : length(ws.trials)
                ...
                intersect_3d_total = [intersect_3d_total; ws.trials{tnum}.whiskerEdgeCoord(noNaNInd,1), ws.trials{tnum}.whiskerEdgeCoord(noNaNInd,2), zeros(length(noNaNInd),1)];
            end
            
            psi1_answer = 'Yes';
            
            while strcmp(psi1_answer, 'Yes')
                disp('Calculating psi for a single ap position session')
                angle_steps_pre = 1 : 185;
                stds_pre = zeros(length(angle_steps_pre),1);
                for i = 1 : length(angle_steps_pre)
                    A = viewmtx(angle_steps_pre(i),0);
                    x4d = [intersect_3d_total, ones(size(intersect_3d_total,1),1)]';
                    x2d = A*x4d;
                ...
                A = viewmtx(psi1(iservo,idist)+90,90);
                x4d = [intersect_3d_total, ones(size(intersect_3d_total,1),1)]';
                x2d = A*x4d;
                h3 = figure('WindowStyle','normal','units','normalized','outerposition',[0 0 1 1]); plot(x2d(1,:), x2d(2,:),'k.', 'MarkerSize',3)        
                answer = MFquestdlg([0.5, 0.3], 'Is psi correct?', 'whisker-pole intersection coordinate scatter', 'Yes', 'No', 'Yes');
                switch answer
                    case 'Yes'
                        close all
...
%                     end
                    ...
                    if isempty(steps_hp{iservo, idist})
                        steps_hp{iservo, idist} = round(-ppm*4 : -ppm*2 + 20);                        
                    end
