function norpix_seq_reader_JK2(seqIn)
...
aviName = [path filesep seqName '.avi'];
...
if totalFrameNumber > nframes_threshold
    ...
    for i = 1 : ceil(totalFrameNumber/nframes_threshold)        
        aviName = [path filesep seqName sprintf('_%02d.avi',i)];
        ...
        mp4CorrectionCMD = sprintf('%s -i %s -b:v 800k -codec:v mpeg4 -c:a copy %s',ffmpegPath, aviName, mp4NameFin);
    ...
    newVid = VideoWriter(aviName, 'Uncompressed AVI');
    open(newVid)
