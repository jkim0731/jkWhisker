        ...
        parfor k=1:nfiles
            try
                ...
                if isempty(p.Results.touch_hp) || isempty(p.Results.hp_peaks)                
                    ...
                    touch_space = [p.Results.touch_hp{th_ind}(1,:) + p.Results.hp_peaks{th_ind}(1) - p.Results.touch_boundary_thickness, ...
                        p.Results.touch_hp{th_ind}(1,:) + p.Results.hp_peaks{th_ind}(2) + p.Results.touch_boundary_thickness; ...
                        p.Results.touch_hp{th_ind}(2:3,:), p.Results.touch_hp{th_ind}(2:3,:)];
%                     th_hull = convhulln(touch_space');                    
                    wl = Whisker.WhiskerTrialLite_2pad(ws,'r_in_mm',p.Results.r_in_mm,'calc_forces',p.Results.calc_forces,...
                        'whisker_radius_at_base',p.Results.whisker_radius_at_base,...
                        'whisker_length',p.Results.whisker_length,'youngs_modulus',p.Results.youngs_modulus,...
                        'baseline_time_or_kappa_value',p.Results.baseline_time_or_kappa_value,...
                        'proximity_threshold',p.Results.proximity_threshold,'pole_pos',pole_pos,'trial_type',trial_type,'th_hull',touch_space');
                end
                outfn = [fn '_WL.mat'];
