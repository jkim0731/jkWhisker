classdef WhiskerTrialArray_2pad < handle
%
%
           ...
           % Still need to add some error checking on inputs.
            ...
            for k=1:length(file_nums)
                ...
                if isempty(trial_types)
                    obj.trials{k} = Whisker.WhiskerTrial_2pad(filename, trial_num, trajectory_nums, obj.mouseName, obj.sessionName);
                else
                    obj.trials{k} = Whisker.WhiskerTrial_2pad(filename, trial_num, trajectory_nums, obj.mouseName, obj.sessionName, trial_types(k));
                end
            end
        end
